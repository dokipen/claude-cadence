...
> **Direct-edit exception:** The lead may use `Edit` directly for mechanical value changes (config literals, wording, version strings) touching a single file. Any change that introduces or removes logic — regardless of line count — must be delegated to an agent. Multi-file changes always require delegation.
...
1. Delegate ALL applicable reviews simultaneously using parallel Agent tool calls:
   ...
   - `performance-engineer` — when the PR touches DB queries, HTTP handlers, file I/O, memory allocation, resource lifecycle, async patterns, or loop-heavy algorithms; **skip** for docs-only, config-only, or agent/skill definition changes
