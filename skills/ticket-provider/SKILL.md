...
- **Global default via `~/.claude/cadence.json`**: Set top-level `provider`, `project_id`, and/or `api_url` fields in `~/.claude/cadence.json` to define a machine-wide default. These apply when the project's `CLAUDE.md` has no `## Ticket Provider` section. This is useful for users who work across many repos all using the same provider. CLAUDE.md always takes precedence over global defaults.
