...
if [ -f "CLAUDE.md" ]; then
  VERIFY_CMD=$(awk 'BEGIN{fence=0} /^```/{fence=!fence; next} !fence && /^## Verification[[:space:]]*$/{while ((getline line) > 0 && (line ~ /^[[:space:]]*$/ || line ~ /^[[:space:]]*#/ || line ~ /^```/)) {}; if (line !~ /^```/) print line; exit}' CLAUDE.md)
fi
