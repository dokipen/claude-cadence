---
name: skill-iterate
description: Evaluate, benchmark, and iteratively improve an existing Gemini CLI skill using Anthropic's skill-creator tooling. Use when a skill draft already exists and the user wants to measure trigger accuracy, run quantitative evals, review output quality, or optimize the skill description. Do NOT use this for creating a skill from scratch — use create-skill for that.
...
```bash
run_shell_command skills/skill-iterate/scripts/install-skill-creator.sh
```
...
The loop covers:
1. write_file or review test prompts for the skill
