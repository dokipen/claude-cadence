...
function createRawClient(url, token) {
    const headers = {};
    if (token) {
...
export function is429Error(error) {
    ...
    return err?.response?.errors?.some((e) => e?.extensions?.code === "TOO_MANY_REQUESTS") ?? false;
}
...
export function getClient() {
    ...
    client.request = async (documentOrOptions, variables, requestHeaders) => {
        let attempt = 0;
        while (true) {
            try {
