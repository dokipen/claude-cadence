...
Users can create, list, inspect, and destroy agent sessions through the gRPC API. Each session launches an agent process in a dedicated PTY-based session.

## Stories

### Create Session
- As a user, I can create a session by specifying an agent profile name
- As a user, I can provide a human-readable session name
...
- As a user, session names are validated (`[a-zA-Z0-9_-]`, max 200 chars)
...
- As a user, the session state reflects reality (reconciled with live PTY processes)
- As a user, if the agent process has exited, the state shows STOPPED
- As a user, I receive NOT_FOUND for unknown session IDs

### Destroy Session
- As a user, I can destroy a session, which terminates the PTY process
...
| Test | Description |
|------|-------------|
| `TestCreateSession_Success` | Create session, verify PTY session is running |
